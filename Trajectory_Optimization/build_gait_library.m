...
fprintf('\n========================= GAIT LIBRARY =========================\n');
fprintf('%8s %10s %8s %10s %8s %8s %6s\n', ...
        'v_des','v_actual','L_step','maxPeriod','rho','cost/L','feas');
for k = 1:numel(gait_library)
    g = gait_library(k);
    fprintf('%8.3f %10.4f %8.3f %10.4f %8.3f %8.1f %6s\n', ...
            g.v_des, g.v_actual, g.step_length, ...
            g.max_periodicity, g.rho, g.cost_norm, tf(g.feasible));
end
fprintf('  (rho < 1 => stable/walkable;  rho >= 1 => falls over)\n');
fprintf('===============================================================\n');
...
function ok = solution_is_feasible(rep, feas_tol)
    ...
    % (stability_c is trivially -1 unless p.enforce_stability is on, so it is
    % harmless to include; note this is CONSTRAINT feasibility, not whether
    % the gait is actually stable -- rep.rho carries that.)
    viol = max([ abs(rep.periodicity(:)); abs(rep.foot_height); ...
                 ...
                 max(0, rep.step_length_c); max(0, rep.stability_c); ...
                 max(0, rep.torque_c); max(0, rep.friction_c); ...
                 max(0, rep.grf_c); max(0, rep.impulse_c) ]);
