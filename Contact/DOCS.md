...
| `rabbit_impact_map.m` | `x_plus = rabbit_impact_map(x_minus)` | **momentum-conserving impact.** Solves the saddle-point system `[M −J_swᵀ; J_sw 0][dq⁺; λ] = [M·dq⁻; 0]` (post-impact swing foot has zero velocity). Returns `x_plus = [q; dq_plus]`. |
