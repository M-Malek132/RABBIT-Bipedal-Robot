...
% "valid" scores exactly that: the steps completed before the first sample at
% which the true contact force lifts off or asks for more than p.limits.mu_s of
% friction (ch4_validity). A run whose "valid" is below its "steps" walked in
% simulation only.
%
% Inputs
...
%                           physical validity (ch4_validity, ch4_run_entry)
...
if opts.verbose
    ...
    fprintf('%s\n', repmat('-',1,105));
    fprintf(' %-11s %6s %7s %6s %6s %9s %9s %10s %10s %9s %7s %8s %7s\n', ...
            'controller', 'scale', 'box', 'steps', 'valid', 'peak|u|', 'over box', ...
            'max|eta|', 'fin|eta|', 'Vend/Vmx', 'QPfail', 'min Fz', 'max mu');
    fprintf('%s\n', repmat('-',1,105));
end
...
for is = 1:numel(scales)
    ...
    for ic = 1:numel(names)
        ...
        if opts.verbose
            fprintf(' %-11s %6.2f %7.0f %6d %6d %9.1f %9.1f %10.3e %10.3e %9.3f %7d %8.0f %7.2f\n', ...
                    entry.name, entry.mass_scale, entry.u_box, ...
                    entry.steps_completed, entry.valid_steps, entry.peak_torque, ...
                    entry.box_violation, entry.max_eta, entry.final_eta, ...
                    entry.V_ratio, entry.qp_infeasible, entry.Fz_min, entry.mu_max);
        end
    end
...
if opts.verbose
    ...
    fprintf([' Read DOWN each controller across scales: the claim is that the\n' ...
             ...
             ' "valid" counts the steps before the first sample that lifts off or\n' ...
             ' asks for more than mu_s = %.2f of friction (ch4_validity).\n'], ...
            p.limits.mu_s);
    fprintf('%s\n\n', repmat('=',1,105));
end
