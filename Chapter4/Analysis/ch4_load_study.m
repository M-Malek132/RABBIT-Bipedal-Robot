...
if opts.verbose
    ...
    fprintf('%s\n', repmat('-',1,105));
    fprintf(' %-9s %-16s %6s %6s %6s %9s %10s %8s %7s %7s %7s  %s\n', 'controller', ...
            'load', 'box', 'steps', 'valid', 'peak|u|', 'max|eta|', 'min Fz', ...
            'bad %', 'max mu', 'QPfail', 'reason');
    fprintf('%s\n', repmat('-',1,105));
end
...
for c = cases
    for k = 1:numel(opts.controllers)
        ...
        if opts.verbose
            fprintf(' %-9s %-16s %6.0f %6d %6d %9.1f %10.3f %8.0f %7.2f %7.2f %7d  %s\n', ...
                    e.name, c.label, e.u_box, e.steps_completed, e.valid_steps, ...
                    e.peak_torque, e.max_eta, e.Fz_min, 100 * e.frac_invalid, ...
                    e.mu_max, e.qp_infeasible, e.reason);
        end
    end
...
if opts.verbose
    fprintf([' "box" binds on clfqp_con and l1_con only. "min Fz" is the TRUE normal\n' ...
             ' force under the load each step actually carried. "valid" counts the steps\n' ...
             ' before the first sample that lifts off or asks for more than mu_s = %.2f\n' ...
             ' of friction; "bad %%" is the share of samples that do (ch4_validity).\n'], ...
            p.limits.mu_s);
    fprintf('%s\n\n', repmat('=',1,105));
end
